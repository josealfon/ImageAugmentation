...
image = io.imread('emergency_vs_non_emergency_dataset/images/0.jpg')
print(image.shape)
io.imshow(image)
...
plt.title('Random Noise')
	# 3.6. Blurring images
blurred = gaussian(imagen, sigma=1, multichannel=True)
plt.imshow(blurred)
...
data = pd.read_csv('emergency_vs_non_emergency_dataset/dataset/emergency_train.csv')
data.head()

	# 2. Loading images
train_img = []
for img_name in tqdm(data['image_names']):
	image_path = 'emergency_vs_non_emergency_dataset/images/' + img_name
...
fig, ax = plt.subplots(nrowns=1, ncols=5,figsize=(20,20))
for i in range(5):
...
# Convet training and validation sets into Pythorc format
final_train = final_train.reshape(7405, 3, 224, 244) 
